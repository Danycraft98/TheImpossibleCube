...
class Control(object):
    ...
    def update(self):
        ...
        self.status_machine.update(self.keys, self.now)

    def draw(self, interpolate):
        if not self.status_machine.state.done:
            self.status_machine.draw(self.screen, interpolate)
            pg.display.update()
            self.show_fps()

    def event_loop(self):
        """
        Process all events and pass them down to the status_machine.
...
def load_all_music(directory, accept=(".wav", ".mp3", ".ogg", ".mdi")):
    """
    Create a dictionary of paths to ost files in given directory

...
from entity import enemy_entity, item_entity
    ...
    def update(self, now, player, group_dict, *args):
        ...
        if self.add_to_map:
            item_groups = (group_dict["items"], group_dict["main"],
                           group_dict["all"])
            item = item_entity.ITEMS[self.item](self.rect, None, True,
...
class Level(object):
    ...
    def __init__(self, player, map_name):
        ...
        self.main_entity = pg.sprite.Group(self.player)
    ...
    def add_map_item(self, event):
        ...
        for target in self.map_dict["Items"]:
            ...
            if keyword == event:
                groups = (self.items, self.main_entity, self.all_group)
                args = target, None, False, (self.name, keyword)
                args += groups
                item_entity.ITEMS[item](*args)

    def spawn(self):
        """Create enemies, adding them to the required groups."""
        groups = (self.enemies, self.main_entity, self.moving, self.all_group)
        for target in self.map_dict["Enemies"]:
            sheet, source, speed = self.map_dict["Enemies"][target]
            enemy_entity.ENEMY_DICT[source](target, speed, *groups)
    ...
    def load_map(self, map_name):
        """Load the map settings from a resource file."""
        path = os.path.join("data", "map_data", map_name)
    ...
    def draw(self, surface, interpolate):
        ...
        for sprite in self.main_entity:
